...
#define HOST_ID SPI3_HOST // the new name of VSPI
	...
	esp_err_t W25Q128_readStatusReg1(W25Q128_t *dev, uint8_t *reg1)
	{
		...
		assert(ret == ESP_OK);
		if (_DEBUG_)
			ESP_LOGI(TAG, "W25Q128_readStatusReg1=%x", data[1]);
		*reg1 = data[1];
		return ret;
	}
	...
	esp_err_t W25Q128_readStatusReg2(W25Q128_t *dev, uint8_t *reg2)
	{
		...
		assert(ret == ESP_OK);
		if (_DEBUG_)
			ESP_LOGI(TAG, "W25Q128_readStatusReg2=%x", data[1]);
		*reg2 = data[1];
		return ret;
	}
	...
	esp_err_t W25Q128_readUniqieID(W25Q128_t *dev, uint8_t *id)
	{
		...
		assert(ret == ESP_OK);
		if (_DEBUG_)
			W25Q128_dump("readUniqueID", data, 13);
		memcpy(id, &data[5], 8);
		return ret;
	}
	...
	esp_err_t W25Q128_readManufacturer(W25Q128_t *dev, uint8_t *id)
	{
		...
		assert(ret == ESP_OK);
		if (_DEBUG_)
			W25Q128_dump("readManufacturer", data, 6);
		memcpy(id, &data[4], 2);
